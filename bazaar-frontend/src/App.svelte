...
<script>
    ...
    function formatNum(num, digits = 2) {
        ...
        if (!isFinite(num)) return num > 0 ? 'Infinite' : num < 0 ? '-Infinite' : 'N/A (Zero Div/Complex)'; // Handle +/- Inf
    ...
    function getBaseIngredientsArray(perspectiveResult) {
        ...
        const ingredientsArray = Object.entries(ingredientsMap).map(([name, detail]) => ({
            ...
            rr: detail.rr,
            if_value: detail.if // Assuming 'if' is the key in JSON from backend
        }));
  ...
  <main>
    ...
    {#if dualResult && !error}
      ...
      {@const secondarySummary = getPerspectiveSummary(secondary, dualResult.quantity)}
        ...
        <div class="results-grid">
           ...
           <div class="perspective">
               ...
               <div class="fill-time-sub-summary">
                  ...
                  {#if primary.calculation_possible && primary.top_level_action && (primary.top_level_action.includes('Expanded') || primary.top_level_action.includes('ExpansionFailed'))}
                      ...
                      {:else if primaryIngredients.length > 0 || (primary.top_level_action === 'ExpansionFailed' && Object.keys(primary.base_ingredients || {}).length > 0)}
                           <p><strong>Ingredient Buy Time:</strong> All ingredients instant or 0s.</p>
                      {:else}
                           <p><strong>Ingredient Buy Time:</strong> No ingredients to buy (e.g. empty recipe or cycle).</p>
                      {/if}
                  ...
                  {:else if !primary.calculation_possible}
                      <p><strong>Ingredient Buy Time:</strong> N/A (Calculation failed)</p>
                  {/if}
               </div>
           </div>
           ...
           <div class="perspective">
                ...
                <div class="fill-time-sub-summary">
                  ...
                  {#if secondary.calculation_possible && secondary.top_level_action && (secondary.top_level_action.includes('Expanded') || secondary.top_level_action.includes('ExpansionFailed'))}
                      ...
                      {:else if secondaryIngredients.length > 0 || (secondary.top_level_action === 'ExpansionFailed' && Object.keys(secondary.base_ingredients || {}).length > 0)}
                           <p><strong>Ingredient Buy Time:</strong> All ingredients instant or 0s.</p>
                      {:else}
                           <p><strong>Ingredient Buy Time:</strong> No ingredients to buy (e.g. empty recipe or cycle).</p>
                      {/if}
        ...
        <!-- Ingredient Tables Grid - NOW VERTICAL -->
                        ...
                        <table>
                            <thead>
                                <tr>
                                    ...
                                    <th class="num-col">IF</th> <!-- New IF Column -->
                                </tr>
                            </thead>
                            <tbody>
                                {#each primaryIngredients as ing}
                                    <tr>
                                        ...
                                        <td class="num-col">{formatNum(ing.if_value, 2)}</td> <!-- Display IF -->
                                    </tr>
                                {/each}
                            </tbody>
                        </table>
                            ...
                            <table>
                                ...
                                <tbody>
                                    {#each Object.entries(secondary.base_ingredients) as [name, detail]}
                                        <tr>
                                            <td>{name}</td>
                                            <td class="num-col">{formatNum(detail.quantity,0)}</td>
                                            <td class="num-col cost-cell">{formatNum(detail.best_cost,2)}</td>
                                            <td>{detail.method}</td>
                                            <td class="num-col">{formatNum(detail.rr, 2)}</td>
                                            <td class="num-col">{formatNum(detail.if, 2)}</td> <!-- Assuming 'if' is the key from backend -->
                                        </tr>
                                    {/each}
                                </tbody>
                            </table>
  ...
  <style>
    main {
      max-width: 1000px; /* Or adjust as needed for single column tables */
      margin: 1.5rem auto;
      padding: 1.5rem;
      background: #1e1e1e;
      border-radius: 8px;
    ...
    .results-grid { /* For summary boxes */
      display: grid;
      grid-template-columns: 1fr 1fr;
      gap: 1.2rem;
      margin-bottom: 1.5rem;
    }
    @media (max-width: 768px) { /* Stack summary boxes on smaller screens */
        .results-grid {
   ...
   .note-info {
    ...
    .ingredient-tables-grid {
        ...
        gap: 1.5rem; /* Increased gap for better separation when stacked */
        margin-top: 1.5rem;
    }
    ...
    .table-wrapper {
      overflow-x: auto; /* Still needed for individual table horizontal scroll if content is too wide */
    }
    table {
      width: 100%;
