...
def calculate_profit_per_hour(recipes, bazaar_prices, auction_prices):
    def get_crafting_details(item_name):
        if item_name not in recipes:
            return None
            
        # Get optimal recipe first
        optimal_recipe = get_optimal_recipe(item_name, recipes, bazaar_prices, auction_prices)
        
        # Skip items that should be bought directly
        if optimal_recipe == {item_name: 1}:
            return None

        total_cost = 0
        total_fill_time = 0
        output_count = recipes[item_name][0].get('count', 1)
        
        # Calculate costs from optimal recipe
        for item, count in optimal_recipe.items():
            bazaar_item = item.replace('-', ':')
            if bazaar_item in bazaar_prices:
                total_cost += bazaar_prices[bazaar_item]['price'] * count
                ...
                total_cost += calculate_bin_tax(auction_prices[item]) * count
            else:
        ...
        sell_price = None
        hourly_volume = 0
        bazaar_id = item_name.replace('-', ':')
        if bazaar_id in bazaar_prices:
            sell_price = bazaar_prices[bazaar_id]['price']
            hourly_volume = bazaar_prices[bazaar_id]['hourly_instabuys']
        elif item_name in auction_prices:
            sell_price = calculate_bin_tax(auction_prices[item_name])
            hourly_volume = float('inf')
        else:
            return None

        crafting_savings = sell_price - (total_cost / output_count)
        if crafting_savings <= 0:
            return None
        
        fill_time = max(total_fill_time, 0.1)
        profit_per_hour = min((3600 / fill_time) * crafting_savings, 
                            crafting_savings * hourly_volume)

        return {
            'item': item_name,
            'profit_per_hour': profit_per_hour,
            'crafting_savings': crafting_savings,
            'fill_time': fill_time,
            'optimal_recipe': dict(optimal_recipe),
            'output_count': output_count
        }

    print("Calculating profitable crafting recipes...")
    profitable_items = []
    
    for item in recipes:
        result = get_crafting_details(item)
        if result:
            profitable_items.append(result)
    
    top_items = sorted(profitable_items, key=lambda x: x['profit_per_hour'], reverse=True)[:40]
    ...
    for item in top_items:
        export_data["top_crafts"].append({
            "item": item['item'],
            "profit_per_hour": item['profit_per_hour'],
            "crafting_savings_per_item": item['crafting_savings'],
            "output_count": item['output_count'],
            "fill_time_seconds": item['fill_time'],
            "recipe": item['optimal_recipe']
        })
    
    with open('top_crafts.json', 'w') as f:
        json.dump(export_data, f, indent=4)
    
    print("\nTop 40 crafts exported to top_crafts.json")
    print("=" * 60)
    for i, item in enumerate(top_items, 1):
        print(f"{i}. {item['item']}")
        print(f"   Profit/hr: {item['profit_per_hour']:,.1f} coins")
        print(f"   Savings/item: {item['crafting_savings']:,.1f} coins")
        print("-" * 60)
