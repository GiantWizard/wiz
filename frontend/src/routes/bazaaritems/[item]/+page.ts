...
export const load: PageLoad = async ({ params, fetch }) => {
  ...
  try {
    const res = await fetch('/api/optimizer_results');
    console.log(`[FULL LOAD +page.ts] Fetch status for /api/optimizer_results: ${res.status}`);

    if (!res.ok) {
        const errorText = await res.text().catch(() => "Could not read error response body");
        console.error(`[FULL LOAD +page.ts ERROR] Failed to fetch /api/optimizer_results: ${res.status} ${res.statusText}. Body: ${errorText}`);
