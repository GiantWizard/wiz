...
# Fetch average price over the last 7 days
def fetch_average_price(item_name, num_entries=7):
    try:
        api_url = f"https://sky.coflnet.com/api/item/price/{item_name}/history/week"
        response = requests.get(api_url)
        response.raise_for_status()
        data = response.json()
        if len(data) < num_entries:
            num_entries = len(data)
        avg_prices = [entry["avg"] for entry in data[-num_entries:]]
...
def build_recipe_tree(data, item_id, prices, lbin_data, visited=None):
    ...
    if bazaar_price > 0:
        if output_count > 1:
            ...
            if bazaar_price < 1200 and total_items >= 40:
...
def compute_weighted_deviation(raw_items, prices):
    # Initialize variables
    total_importance = 0
    weighted_deviation_sum = 0
    item_data_map = {}

    # First pass: gather all data and calculate deviations
    for item, quantity in raw_items.items():
        # Fetch 7-day average price for the item
        avg_price = fetch_average_price(item)
        if not avg_price:
            continue
        ...
        hourly_instasells = price_info["hourly_instasells"]
        ...
        try:
            if method == "sell_order":
                importance = current_price * quantity
            else:  # Buy Order
                if hourly_instasells <= 0:
                    continue
                importance = (current_price * quantity) + (quantity / hourly_instasells)
        except ZeroDivisionError:
            continue
    ...
    # Second pass: calculate weighted deviations
    if total_importance > 0:
        for item, data in item_data_map.items():
            weight = data["importance"] / total_importance
            weighted_deviation = data["deviation"] * weight
            weighted_deviation_sum += weighted_deviation

    return weighted_deviation_sum
...
def calculate_profit(data, prices, lbin_data):
    ...
    for item_id in data.keys():
        ...
        if profit > 50000 and hourly_instabuys > 0:
            ...
            weighted_dev = compute_weighted_deviation(raw_items, prices)
            ...
            profits.append({
                ...
                "total_time": 1 / hourly_instabuys if hourly_instabuys > 0 else float('inf'),
...
try:
    ...
    for craft in top_crafts:
        output.append({
            ...
            "weighted_deviation": craft["weighted_deviation"]  # Fixed key name
        })
