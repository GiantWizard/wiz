...
func calculateC10MInternal(
	...
	if deltaRatio > 1.0 { // More supply than demand pressure: order likely fills fast
		...
		if math.IsInf(rrValue, 1) { // If RR is infinite, primary cost is infinite
			...
			// This part of the C10M formula isn't fully documented publicly, so this is an
			// approximation based on observed bazaar behavior: extra = sellP * (qty*rrValue -
			// ifValue*sumK), where sumK is the sum of integers from 1 to RR (rounded). It's meant
			// to approximate the extra cost from having to relist an order multiple times before
			// it fully fills.
			...
			if adjustment > 0 { // Only calculate if there's an adjustment
				...
				// The term (qty*rrValue - ifValue*sumK) can go negative if ifValue*sumK is large,
				// which would imply a "gain" rather than a cost, so it gets clamped to 0 below.
