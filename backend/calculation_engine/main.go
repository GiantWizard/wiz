...
func downloadMetricsFromMega(localTargetFilename string) error {
	...
	runCmd := func(name string, args ...string) (string, error) {
		ctx, cancel := context.WithTimeout(context.Background(), megaCmdTimeout)
		defer cancel()
	...
	lsOutput, lsErr := runCmd(megaLsCmd, remoteDir)
	if lsErr != nil {
		return fmt.Errorf("%s %q failed: %v\nOutput:\n%s", megaLsCmd, remoteDir, lsErr, lsOutput)
	}
	...
	getOut, getErr := runCmd(megaGetCmd, remoteFile, "--path", targetDir)
	if getErr != nil {
		return fmt.Errorf("%s %q failed: %v\nOutput:\n%s", megaGetCmd, remoteFile, getErr, getOut)
	}
...
// latestMetricFileHandler finds the most recently generated metrics file from the Rust
// service and returns its JSON content directly.
