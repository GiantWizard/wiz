...
type OptimizedItemResult struct {
	...
	CostAtOptimalQty            float64  `json:"cost_at_optimal_qty"`
	RevenueAtOptimalQty         float64  `json:"revenue_at_optimal_qty"`
	MaxProfit                   float64  `json:"max_profit"`
	TotalCycleTimeAtOptimalQty  float64  `json:"total_cycle_time_at_optimal_qty"`
...
func optimizeItemProfit(
	...
	if result.MaxFeasibleQuantity <= 0 {
		...
		if errCheckQty1 != nil {
			...
			result.TotalCycleTimeAtOptimalQty = sanitizeFloat(totalTime)
	...
	result.TotalCycleTimeAtOptimalQty = sanitizeFloat(acqTimeFinal + saleTimeFinal)
