...
string safeSystem(const string& cmd, bool checkError = true, const vector<int>& allowedExitCodes = {}) {
    ...
    if (!pipe) { throw runtime_error("Failed to execute popen for command: " + effective_cmd); }

    stringstream output_stream;
    char buffer[256];
    while (fgets(buffer, sizeof(buffer), pipe) != NULL) { output_stream << buffer; }
    ...
    if (exit_code != -1) { for (int allowed_code : allowedExitCodes) { if (exit_code == allowed_code) { isAllowedExitCode = true; break; } } }

    if (checkError && exit_code != 0 && !isAllowedExitCode) {
        string error_msg = "Export Engine: Command [" + effective_cmd + "] failed ";
        if (exit_code != -1) { error_msg += "with exit code " + to_string(exit_code); }
        else { error_msg += "(abnormal termination, status: " + to_string(status) + ")"; }
        if(!cmd_output_str.empty()) { error_msg += ". Output was: " + cmd_output_str; }
...
int main(int argc, char* argv[]) {
    ...
    try {
        // The call to ensureRemoteDirExists has been removed.
        
        // This command now uses the full remote path, which tells `mega-put`
        // to rename the file upon upload to the root directory.
