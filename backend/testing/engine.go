...
func aggregateCells(cells map[string]string) map[string]int {
	...
	for _, pos := range positions {
		...
		ing := parts[0] // keep original casing
...
// path holds the recursion chain (item name and factor) to detect cycles.
func expandItem(itemName string, factor int, path []ItemStep) map[string]int {
	...
	// Check if itemName already exists in the path (cycle detection)
	for _, step := range path {
		if step.name == itemName {
			// Instead of returning current factor, go back 2 steps.
			if len(path) >= 2 {
			...
			// If not enough history, just treat as base.
	...
	for ing, amt := range aggregated {
		...
		log.Printf("Expanding ingredient '%s' with new factor %d (base factor %d * ingredient amount %d)",
...
func main() {
	...
	for {
		...
		if strings.TrimSpace(itemName) == "exit" {
			break
		}
